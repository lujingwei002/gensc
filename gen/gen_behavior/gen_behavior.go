...
import (
	...
	"log"
	"os"
	"github.com/lujingwei002/gensc/behavior"
...
func (self *<<.MongoDaoStructName>>) Sync(ctx context.Context, opts ...behavior.SyncOption) (n int, err error) {
	...
	if err != nil {
		...
		log.Printlnw("sync behavior", "name", "<<.CollName>>", "len", len(models))
		n = len(models)
		return 
	}
}
...
type gen_state struct {
	config      gensc.GenBehaviorConfig
...
func Gen(config gensc.GenBehaviorConfig) error {
	log.Println("===============gen behavior start===============")
	state := &gen_state{
		config:      config,
	...
	log.Println("===============gen behavior finished===============")
	return nil
}
