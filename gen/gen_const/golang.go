...
func (p *golang_parser) parseConstStruct(state *gen_state, fset *token.FileSet, filePath string, s *ast.StructType) error {
...
func (p *golang_parser) parse(state *gen_state) (err error) {
	filePath := path.Join(state.config.DocDir, "const.go")
...
func (p *golang_parser) parseConstsStruct(state *gen_state, filePath string, fileContent []byte, fset *token.FileSet, g *ast.GenDecl, t *ast.TypeSpec, s *ast.StructType) error {
	if g.Doc == nil {
