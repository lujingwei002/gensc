...
func (p *golang_parser) parse(state *gen_state) error {
	...
	if err := filepath.WalkDir(path.Join(state.config.DocDir), func(path string, d os.DirEntry, err error) error {
		if d == nil {
			return nil
		}
		if d.IsDir() {
	...
	if annotations, err := p.extraBehaviorsAnnotate(fset, filePath, s, t.Name.Name, g.Doc.List); err != nil {
		...
		if v, ok := state.databasDict[dbName]; !ok {
			database = &Database{
				...
				GenXmlDir:           path.Join(state.config.GenXmlDir, dbName),
				GenXmlFilePath:      path.Join(state.config.GenXmlDir, dbName, fmt.Sprintf("%s.xml", dbName)),
				SrcGenModelDir:      path.Join(state.config.SrcGenDir, dbName),
				SrcGenModelFilePath: path.Join(state.config.SrcGenDir, dbName, fmt.Sprintf("%s.gen.go", dbName)),
				SrcGenBinFilePath:   path.Join(state.config.SrcGenDir, dbName, "bin", fmt.Sprintf("%s.gen.go", dbName)),
				SrcGenBinDir:        path.Join(state.config.SrcGenDir, dbName, "bin"),
				DbName:              dbName,
