...
func (p *golang_parser) parse(state *gen_state) error {
	...
	if d, err := os.Open(state.config.DocProtocolDir); err != nil {
		return err
	} else {
	...
	for _, name := range nameArr {
		dir := filepath.Join(state.config.DocProtocolDir, name)
		protocolFilePathArr := make([]string, 0)
		protocolFilePath := filepath.Join(state.config.DocProtocolDir, fmt.Sprintf("%s.go", name))
