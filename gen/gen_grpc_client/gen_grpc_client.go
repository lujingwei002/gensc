...
type serviceGenerateHelperInterface interface {
	...
	generateUnimplementedServerType(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, service *protogen.Service, opts Options)
...
func (serviceGenerateHelper) generateUnimplementedServerType(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, service *protogen.Service, opts Options) {
	serverType := service.GoName + "Server"
	mustOrShould := "must"
	if !*opts.RequireUnimplemented {
		mustOrShould = "should"
	}
...
func GenerateFile(gen *protogen.Plugin, file *protogen.File, opts Options) *protogen.GeneratedFile {
	...
	g.P("// versions:")
	g.P("// - protoc-gen-go-gclient v", opts.Version)
	...
	generateFileContent(gen, file, g, opts)
	return g
}
...
func generateFileContent(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, opts Options) {
	if len(file.Services) == 0 {
		return
	}
	...
	for _, service := range file.Services {
		genService(gen, file, g, service, opts)
	}
}

func genService(gen *protogen.Plugin, file *protogen.File, g *protogen.GeneratedFile, service *protogen.Service, opts Options) {
	...
	// Server Unimplemented struct for forward compatibility.
	helper.generateUnimplementedServerType(gen, file, g, service, opts)
