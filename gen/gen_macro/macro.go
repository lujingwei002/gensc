...
func scanDirFiles(state *GenState) map[string][]string {
	dirArr := make([]string, 0)
	dirArr = append(dirArr, state.config.SrcDir)
	if state.config.SrcDirs != nil {
		dirArr = append(dirArr, state.config.SrcDirs...)
	}
	files := make(map[string][]string, 0)
	for _, dir := range dirArr {
		filepath.WalkDir(dir, func(path string, d os.DirEntry, err error) error {
			if strings.HasPrefix(path, state.config.SrcGenDir) {
			} else if strings.HasPrefix(path, state.config.SrcTestDir) {
...
func Gen(config gensc.MacroConfig) error {
	log.Println("===============gen macro start===============")
	log.Println(config.SrcDirs)
	state := &GenState{
		config: config,
	}
	files := scanDirFiles(state)
...
/// =============宏展开的地方，不要在文件末尾添加代码============`)
	...
	log.Println("===============gen macro finished===============")
	return nil
}
