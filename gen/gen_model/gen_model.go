...
var (
	e = errors.New("imported and not used")
)
...
func Use(ctx context.Context, client gensc.DbClient) (<<.DaoInterfaceName>>, error) {
...
func UseMongo(ctx context.Context, client gensc.MongoClient) (*<<.MongoDaoStructName>>, error) {
	dao := NewMongoDao()
...
func UseRedis(ctx context.Context, client gensc.RedisClient) (*<<.RedisDaoStructName>>, error) {
	dao := NewRedisDao()
	if err := dao.UseClient(client); err != nil {
		return nil, err
	}
...
func Migrate(ctx context.Context, client  gensc.DbClient, opts ...db.MigrateOption) error {
...
func (self *<<.ArrStructName>>) Add(<<.CapCamelSecondaryKey>> <<.SecondaryKeyField.GoTypeName>>) (*<<.StructName>>, error) {
	...
	if _, ok := self.dict[<<.CapCamelSecondaryKey>>]; ok {
		return nil, errors.New("<<.ArrStructName>> secondary key conflict <<.CapCamelSecondaryKey>>")
	}
...
func (self *<<.MongoDaoStructName>>) UseClient(client gensc.DbClient) error {
	switch c := client.(type) {
	case gensc.MongoClient:
...
func (self *<<.MongoDaoStructName>>) Save(ctx context.Context, doc *<<.StructName>>) error {
	...
	if result, err := coll.ReplaceOne(ctx, bson.D{{"_id", doc.Id}}, (*doc).<<.DataStructName>>, opts); err != nil {
		...
		if doc.none && result.MatchedCount != 0 {
			log.Println("<<.CollName >> matched count is not zero, %+v", doc)
		} else if !doc.none && result.MatchedCount != 1 {
			log.Println("<<.CollName >> matched count is zero, %+v", doc)
		}
		if result.ModifiedCount + result.UpsertedCount != 1 {
			log.Println("<<.CollName >> unchanged, %+v", doc)
		}
	}
	doc.dirty = false
	return nil
...
func (self *<<.MongoDaoStructName>>) Delete(ctx context.Context, doc *<<.StructName>>) error {
	if doc == nil {
		return gensc.ErrNullPointer
	}
	if doc.none {
		return gensc.ErrDataNotExist
	}
	if doc.Id.IsZero() {
		return gensc.ErrDataNotExist
	}
	...
	if result, err := coll.DeleteOne(ctx, bson.D{{"_id", doc.Id}}); err != nil {
		...
		if result.DeletedCount != 1 {
			return gensc.ErrDataDeleteFail
		}
	}
	doc.dirty = false
	doc.none = true
...
func (self *<<.RedisDaoStructName>>) UseClient(client gensc.DbClient) error {
	switch c := client.(type) {
	case gensc.RedisClient:
...
func Gen(config gensc.GenModelConfig) error {
	log.Println("===============gen model start===============")
	state := &gen_state{
		config:      config,
		databaseArr: make([]*Database, 0),
		databasDict: make(map[string]*Database),
	}
	if _, err := os.Stat(state.config.SrcGenDir); err != nil {
		if os.IsNotExist(err) {
	...
	log.Println("===============gen model finished===============")
	return nil
}
