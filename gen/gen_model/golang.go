...
func (p *golang_parser) parse(state *gen_state) error {
	...
	filepath.WalkDir(state.config.DocDir, func(path string, d os.DirEntry, err error) error {
		if d == nil {
			return nil
		}
		if d.IsDir() {
	...
	if annotations, err := p.extraModelsAnnotate(fset, filePath, s, t.Name.Name, g.Doc.List); err != nil {
		...
		if v, ok := state.databasDict[dbName]; !ok {
			database = &Database{
				...
				GenBinFilePath:      path.Join(state.config.SrcGenDir, dbName, "bin", fmt.Sprintf("%s.gen.go", dbName)),
				GenBinDir:           path.Join(state.config.SrcGenDir, dbName, "bin"),
				GenModelDir:         path.Join(state.config.SrcGenDir, dbName),
				GenModelFilePath:    path.Join(state.config.SrcGenDir, dbName, fmt.Sprintf("%s.gen.go", dbName)),
				GenProtobufFilePath: path.Join(state.config.GenProtoDir, dbName, fmt.Sprintf("%s.gen.proto", dbName)),
				DbName:              dbName,
