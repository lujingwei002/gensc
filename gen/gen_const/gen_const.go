...
import (
	"github.com/lujingwei002/gensc/codes"
)
...
func genConstFile(state *gen_state, descriptor *Descriptor, data *excel_data) error {

	log.Println("gen const ", descriptor.Name)
	dir := path.Join(state.config.SrcGenDir, descriptor.Name)
...
func parse(state *gen_state) error {
	...
	// 读取excel文件
	for name, v := range state.constFile.descriptorDict {
		filePath := path.Join(state.config.ExcelDir, v.filePath)
		resource := &excel_data{
			Descriptor: v,
		...
		state.excelDataDict[name] = resource
	}
	return nil
}

func genCode(state *gen_state) error {
	log.Println("生成go文件")
	// 生成cost文件夹
	dir := state.config.SrcGenDir
	if err := os.RemoveAll(dir); err != nil {
		return err
	}
	if err := os.Mkdir(dir, 0755); err != nil {
	...
	// 生成代码
	for name, descriptor := range state.constFile.descriptorDict {
		if resourceData, ok := state.excelDataDict[name]; ok {
			if err := genConstFile(state, descriptor, resourceData); err != nil {
				return err
			}
		} else {
...
// 生成协议
func Gen(config gensc.GenConstConfig) error {
	log.Println("===============gen const start===============")
	// 初始化
	constState := &gen_state{
		config:        config,
	...
	log.Println("===============gen const finished===============")
	return nil
}
