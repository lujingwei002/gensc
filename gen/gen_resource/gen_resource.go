...
import (
	...
	"os"
	"fmt"
	"github.com/lujingwei002/gensc/db"
	"github.com/lujingwei002/gensc/resource_options"
...
import (
	...
	"github.com/lujingwei002/gensc/log"
	"os"
	"fmt"
	"context"
<<range .ImportArr>>
...
// dir - bundle所在的目录
func Push(ctx context.Context, client gensc.DbClient, dir string, opts ...resource_options.PushOption) error {
...
func New<<.LoaderStructName>>(handler gensc.ResourceHandler) *<<.LoaderStructName>>{
...
// 从db中加载资源
func (self *<<.LoaderStructName>>) LoadFromDb(ctx context.Context, client gensc.DbClient) error {
	<<- range .BundleArr>>
...
func (self *<<.LoaderStructName>>) LoadResource(ctx context.Context, client gensc.DbClient, dir string, compress bool) error {
...
// 根据bundle的类型，从相应的源中加载资源
func (self *<<.LoaderStructName>>) ReloadResource(ctx context.Context, client gensc.DbClient, dir string, compress bool) error {
	if err := self.LoadVersion(dir); err != nil {
		return err
	}
	<<- range .BundleArr>>
...
// 加载成功后，对配置进行处理
func (self *<<.LoaderStructName>>) convert(handler gensc.ResourceHandler) error {
...
// 从db中加载资源
func (self* <<.BundleStructName>>) LoadFromDb(ctx context.Context, client gensc.DbClient) error {
	self.Clear()
	<<- range .ResourceArr>>
...
func (self *<<.BundleStructName>>) SaveToDb(ctx context.Context, client gensc.DbClient, dir string, opts ...resource_options.PushOption) error {
	switch c := client.(type) {
	case gensc.MongoClient:
...
func (self *<<.ArrTypeName>>) LoadFromYaml(filePath string) error {
	...
	if err := yaml.Unmarshal(data, self); err != nil {
		log.Warnw("load resource fail", "name", filePath, "error", err)
		return err
	}
	return nil
}
...
func (self *<<.ArrTypeName>>) LoadFromDb(ctx context.Context, client gensc.DbClient) error {
	switch c := client.(type) {
	case gensc.MongoClient:
		return self.LoadFromMongo(ctx, c.GetMongoDatabase())
	default:
		panic(gensc.ErrDbNotSupport)
	}
}
...
func (self *<<.WrapStructName>>) LoadFromDb(ctx context.Context, client gensc.DbClient) error {
	switch c := client.(type) {
	case gensc.MongoClient:
		return self.LoadFromMongo(ctx, c.GetMongoDatabase())
	default:
...
func (self *<<.WrapStructName>>) Make(arr <<.ArrTypeName>>) error {
	if err := gensc.Make<<len .MapKeyArr>>Key_<<join .MapKeyGoTypeNameArr "_">>(arr, *self<<- range $k, $v := .MapKeyArr>>, "<<camelString $v>>"<<- end>>); err != nil {
		return err
	}
	return nil
}
...
func (self *<<.WrapStructName>>) Make(arr <<.ArrTypeName>>) error {
	...
	if err := gensc.Make<<len .ObjectKeyArr>>Key_<<join .ObjectKeyGoTypeNameArr "_">>(arr, dict<<- range $k, $v := .ObjectKeyArr>>, "<<camelString $v>>"<<- end>>); err != nil {
		return err
	}
	var ok bool
	<<- range .ValueArr>>
...
type gen_state struct {
	Config       gensc.GenResourceConfig
	Module       string
	...
	for index, v := range nameRow {
		if v != "" {
			...
			if realType, ok := type_name_dict[typeRow[index]]; ok {
				...
				log.Println("invalid type", "file_path", filePath, "type", typeName, "field", v)
...
func genResourcesVersion(state *gen_state) error {
	log.Println("生成version文件")
	filePath := filepath.Join(state.Config.ResourceDir, "conf", ".version.yaml")
	v := version_file{}
...
func genXml(state *gen_state) error {
	log.Println("生成xml文件")
	if _, err := os.Stat(state.Config.GenDir); os.IsNotExist(err) {
		if err := os.Mkdir(state.Config.GenDir, 0755); err != nil {
			return err
		}
	}
	for _, bundle := range state.BundleArr {
		if bundle.BundleType == "db" {
			var err error
			filePath := path.Join(state.Config.GenDir, fmt.Sprintf("%s.xml", bundle.BundleName))
			file, err := os.OpenFile(filePath, os.O_WRONLY|os.O_CREATE, 0644)
			if err != nil {
				return err
			}
...
func genResourcesYaml(state *gen_state) error {
	log.Println("生成yaml文件")
	if _, err := os.Stat(state.Config.ResourceDir); os.IsNotExist(err) {
		if err := os.Mkdir(state.Config.ResourceDir, 0755); err != nil {
			return err
		}
	}
	for _, bundle := range state.BundleArr {
		for name, v := range bundle.ResourceArr {
			log.Println(name, "==>", path.Join(bundle.BundleName, v.YamlFileName))
			if _, err := os.Stat(path.Join(state.Config.ResourceDir, "conf")); os.IsNotExist(err) {
				if err := os.Mkdir(path.Join(state.Config.ResourceDir, "conf"), 0755); err != nil {
					return err
				}
			}
			if _, err := os.Stat(path.Join(state.Config.ResourceDir, "conf", bundle.BundleName)); os.IsNotExist(err) {
				if err := os.Mkdir(path.Join(state.Config.ResourceDir, "conf", bundle.BundleName), 0755); err != nil {
					return err
				}
			}
			filePath := path.Join(state.Config.ResourceDir, "conf", bundle.BundleName, v.YamlFileName)
			file, err := os.OpenFile(filePath, os.O_WRONLY|os.O_CREATE, 0644)
			if err != nil {
				return err
			}
	...
	// 	log.Println(name, "==>", v.YamlFileName)
...
func genResourcesLoader(state *gen_state) error {
	log.Println("生成go文件")
	if _, err := os.Stat(state.Config.SrcGenDir); err != nil && os.IsNotExist(err) {
		if err := os.Mkdir(state.Config.SrcGenDir, 0755); err != nil {
			return err
		}
	}
	funcMap := template.FuncMap{
	...
	resourcesPath := path.Join(state.Config.SrcGenDir, "resource.gen.go")
...
func genResourceCli(state *gen_state) error {
	if _, err := os.Stat(path.Join(state.Config.SrcGenDir, "bin")); err != nil && os.IsNotExist(err) {
		os.Mkdir(path.Join(state.Config.SrcGenDir, "bin"), 0755)
	}
	resourceFilePath := path.Join(state.Config.SrcGenDir, "bin", "resource.gen.go")
...
// 生成协议
func Gen(config gensc.GenResourceConfig) error {
	log.Println("===============gen resource start===============")
	// 初始化
	state := &gen_state{
		Config:       config,
	...
	if err := p.parse(state); err != nil && err == gensc.ErrGenNotChange {
		log.Println("===============gen resource finished, not change===============")
		return nil
	} else if err != nil {
		return err
	}
	...
	log.Println("===============gen resource finished===============")
	return nil
}
