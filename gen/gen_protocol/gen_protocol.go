...
func (self *Client) readRoutine() error {
	...
	for {
		...
		if err != nil {
			//log.Println("client read routine exit", err)
...
type gen_state struct {
	protocols []*protocol
	config    gensc.GenProtocolConfig
}
...
func genSproto(state *gen_state, proto *protocol) error {
	dir := filepath.Join(state.config.GenProtocolDir, proto.Module)
	if _, err := os.Stat(state.config.GenProtocolDir); os.IsNotExist(err) {
		if err := os.Mkdir(state.config.GenProtocolDir, 0755); err != nil {
			return err
		}
	}
...
func genSprotoGo(state *gen_state, proto *protocol) error {
	dir := filepath.Join(state.config.SrcGenProtocolDir, proto.Module)
	if _, err := os.Stat(state.config.SrcGenProtocolDir); os.IsNotExist(err) {
		if err := os.Mkdir(state.config.SrcGenProtocolDir, 0755); err != nil {
			return err
		}
	}
	if _, err := os.Stat(dir); os.IsNotExist(err) {
...
func genSprotoGo2(state *gen_state, proto *protocol) error {
	dir := filepath.Join(state.config.SrcGenProtocolDir, proto.Module)
	if _, err := os.Stat(state.config.SrcGenProtocolDir); os.IsNotExist(err) {
		if err := os.Mkdir(state.config.SrcGenProtocolDir, 0755); err != nil {
			return err
...
// 生成协议
func Gen(config gensc.GenProtocolConfig) error {
	log.Println("===============gen protocol start===============")
	...
	log.Println("===============gen protocol finished===============")
	return nil
}
